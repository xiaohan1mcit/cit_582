...
def hash_collision(k):
    ...
    x_size = len(hex_result_x_random) * 4
    bin_result_x_random = (bin(int(hex_result_x_random, 16))[2:]).zfill(x_size)

    # get last k digits of the hash
    last_k = bin_result_x_random[-k:]
    ...
    while True:
        ...
        if (bin_result_y_random[-k:] == last_k) :
            y = byte_str_y_random
            break
